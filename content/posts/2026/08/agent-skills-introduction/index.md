...
AI コーディングエージェントにデプロイ手順、データベースマイグレーション、障害調査などの定型タスクを実行させる際、すべての詳細手順を `AGENTS.md` などのルールファイルに記述すると、プロンプトが肥大化して **Context Bloat（トークン消費の増大）** が発生します。

さらに、スタンフォード大学などの研究 **[『Lost in the Middle』(Liu et al., 2023)](https://arxiv.org/abs/2307.03172)** が実証した通り、プロンプトが長大化するほど中盤に書かれた指示への注意（Attention）が著しく低下する **Attention Dilution（重要指示の埋没・見落とし）** が発生します。
