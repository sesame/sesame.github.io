...
flowchart TD
    ...
    -->|"手足と手順の分離・標準化"| G2["【第2世代】MCP & Skills<br>・MCP：ツール接続のAPI標準<br>・Skills：手順書のオンデマンド開示"]
...
- **Rules（`AGENTS.md`）と Skills の限界**:
  Rules や Skills はあくまで **「自然言語（お願い）」** です。「危険なコマンドを実行するな」「直接 push するな」とどれだけ指示しても、LLM の確率的な挙動により 100% 確実に防ぐことは困難でした。
- **Hooks のアプローチ（決定論的制御）**:
  LLM の自制心に頼るのをやめ、エージェントの動作ループに **シェルスクリプトを強制的に割り込ませる仕組み（Git hooks と同じ発想）** です。
...
| **Skills (`SKILL.md`)** | **標準化済み** | agentskills.io | 業務マニュアル・手順書のオンデマンド規格 |
