...
各ツールが担う役割は以下の通りです。それぞれ単一責任のシンプルな CLI ツールをパイプラインで繋ぎ合わせます。

| ツール | 役割 | 解決する課題 |
| :--- | :--- | :--- |
| **`ghq`** | Git リポジトリの一元管理 | リポジトリの配置場所（ディレクトリ散乱）の統一 |
| **`peco`** | 標準入力のインクリメンタルフィルタリング | 候補一覧からのあいまい検索・選択 |
| **`fish`** | シェル環境・キーバインド・補完 | シンタックスハイライト、オートサジェスト、関数の定義 |
| **`tmux`** | ターミナルセッションの永続化・画面分割 | サーバー/ワーカープロセスの維持、プロジェクトごとの作業空間分離 |
