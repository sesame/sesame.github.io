...
> [!TIP]
> **単一指示ファイルの肥大化が引き起こす 2 つの技術的ボトルネック**
> `AGENTS.md` や `CLAUDE.md` に過去のエラー対策をすべて書き足していくアプローチには、以下の欠陥があります。
> 
> 1. **Context Bloat（トークン消費の爆発）**:
>    大量の知見が毎ターン送信され、何もしなくても会話開始時点で大量の入力トークンを消費し、レイテンシと API コストが急増します。
> 2. **Attention Dilution / "Lost in the Middle" 現象（指示の希釈）**:
>    スタンフォード大等の研究 **[『Lost in the Middle』(Liu et al., 2023)](https://arxiv.org/abs/2307.03172)** の通り、プロンプトが長大化・ノイズ化するほど重要指示に対する注意が希釈され、最重要ルールの遵守率が低下します。
...
> [!TIP]
> **常時ロード層 vs オンデマンド参照層の仕組み**
> - **`AGENTS.md`（常時強制ロード）**:
>   エージェントツールの基盤仕様として、毎ターンのプロンプトに無条件で自動注入されます。
> - **`Learnings.md`（オンデマンド参照）**:
>   システムが全行を自動注入することはありません。`AGENTS.md` のポインタ指示に従い、エージェントが必要な時だけツール（`view_file` や `grep_search`）で**能動的に検索・読み込みを行う通常ファイル**として扱われます。
...
> [!TIP]
> **知見のライブラリ化（Skills への昇格）**
> `Learnings.md` に蓄積された断片的な教訓のうち、定型化・複雑化したワークフローは **`Skills`（`.agents/skills/*/SKILL.md`）** へ昇格させます。
> スキルは「名前と概要（メタデータ）」のみを軽量保持し、**該当タスク実行時のみ全文がオンデマンド注入される**ため、トークンを浪費することなく高度な専門手順をライブラリ化できます。


> [!NOTE]
> **外部記憶アーキテクチャの学術的ルーツと実務背景**
> 
...
> - **実務への定着**:
>   障害対応の事後検証（Post-mortem）から得られる「Key Learnings」文化と、Addy Osmani 氏の「Memory Primitive（*The agent forgets, the repo doesn't*）」が接続し、2 層構造が自律ループの基本設計として定着。
...
日常的に使用するテスト・ビルド・Git コマンドを事前許可（Allowlist 化）し、**「人間の確認待ちによるループ停止」を排除** して AI を止まらずに自走させます。

> [!WARNING]
> **「言葉による確認スキップ」と「物理サンドボックス」の決定的な違い**
> 単にプロンプトで「確認してこないで勝手に進めて」と指示したり、全権限を無制限に解放（`--dangerously-skip-permissions`）するのは、**セーフティネットのない危険なノールック運転（YOLO）** に過ぎません。
> 
> また、「Git があればコードを壊されても戻せる」「テストコマンドだけ許可（Allowlist）すれば安全」という考えにも落とし穴があります。
> たとえば `npm test` や `pytest` を許可していたとしても、**サンドボックスがないホスト環境ではテストプロセス自体がホスト OS の権限で動くため、テストスクリプト内でホスト側の重要ファイルを削除・破壊されるリスク** を防げません。
> 
> - **Git**: 「リポジトリ内のコードの可逆性」しか保証しない
> - **CLI の Allowlist**: アプリケーション層の制約に過ぎず、プロセス内部の挙動は防げない
> - **サンドボックス（Docker / WSL2 等）**: **OS カーネルレベルでホストからプロセスを物理隔離する**
> 
> だからこそ、自律ループの権限を解放するには、**「最悪壊れてもホストに影響せず、即座に捨てて再構築できる物理サンドボックス（ハーネス）」** が絶対の大前提となります。
...
> [!TIP]
> **Fast Observability（高速な自動検証）を成立させる 4 要件**
> 1. **5 秒以内の高速実行**: 検証が遅いと自己修復ループ全体の回転が鈍化します。
> 2. **厳格な終了コード（Exit Code 0/1）**: 成功/失敗を AI が機械的に判定できるようにします。
> 3. **明瞭な標準エラー出力（stderr）**: エラー行・スタックトレースから AI が自力で原因特定できるようにします。
> 4. **知見の正確性フィルター**: 自動テストをパスして再現性が証明された解決策だけを `Learnings.md` に蓄積します。
...
```markdown
# 自律動作ポリシー
- **自律完遂**: 軽微な判断やエラー自己修復は質問を挟まず自律的に進める。破壊的変更（ファイル破棄や強制プッシュ等）のみ確認を行う。
- **知見の参照**: 詳細な技術仕様や過去のトラブルシューティングは `Learnings.md` を参照する。
- **自己進化**:
  - 新たなトラブル解決や環境固有の仕様・ノウハウを発見した際は、自発的に `Learnings.md`（および必要に応じて本 `AGENTS.md`）へ要約追記してタスクを完了する。
  - `Learnings.md` の最終整理日から1週間以上経過している場合、内容を棚卸し・整理して日付を当日の日付に更新する。
```
...
### 1. ハーネス（物理サンドボックス）の必須性と非隔離環境でのトレードオフ

AI は確率的・非決定的に動作するため、プロンプトや CLI ツールの設定（ソフトウェア制御）だけで予期せぬ挙動を 100% 防ぐことは原理的に不可能です。そのため、自律ループの構築には OS カーネルレベルでプロセスを封じ込める **「物理サンドボックス（Docker、WSL2、使い捨て VM 等）」が不可欠な前提** となります。

しかし、実務環境（特にコンテナ導入に制約がある Windows ホスト等）において物理サンドボックスを用意できない場合、**「安全性と自律性の厳しいトレードオフ」** に直面します。

- **自律性を追求する場合（サンドボックスが必須）**:
  ホスト OS を破壊するリスクを排除するため、最低限の隔離環境（WSL2 や Windows Sandbox、独立 VM）を用意してその中でエージェントを動かす必要があります。
- **物理隔離がない場合（安全のために自律性を制限）**:
  Allowlist による自動実行はホスト破壊のリスクを伴うため、コマンド実行のたびに人間が目視確認する **「Human in the loop の多発（自律性の犠牲）」を受け入れるのがセキュリティ上極めて正当な判断** となります。

また、Docker 等でサンドボックスを構築した場合でも、ホストマウントされたファイルの誤削除、API トークンの漏洩、リモート Git の誤プッシュなど「100% 完璧な安全」の保証は困難です。安全性を過信せず、破壊的変更への人間確認を残すなどの多重防御（Defense in Depth）が求められます。
