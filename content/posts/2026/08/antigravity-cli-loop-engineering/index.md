...
その理由は、**「人間が間に入ってプロンプトを入力し、目視確認する 1 問 1 答の運用では、スループットに構造的な限界があるため」** です。

- **人間のタイピングと目視速度が「開発速度のボトルネック」になる**:
  AI のモデル知能が向上しても、人間が指示を入力し、出力を確認して修正を指示している限り、全体の生産性は「人間の手動介入速度」で頭打ちになります。
- **手動確認による認知負荷の蓄積**:
  AI の出力コードを人間が毎回目で追い、不備を見つけてプロンプトで修正させる作業は認知負荷が高く、スケールしません。
- **初期投資の先にあるスループットの向上**:
  ハーネスやテストの整備にはコストがかかりますが、一度「自律ループ」を構築すれば、人間が他の作業を進めている間に AI が自走してタスクを完走する高いスループットが得られます。

これは、かつて **CI/CD（継続的インテグレーション・自動テスト）** が登場した際、「テストを書くのが面倒」「CI の構築コストがかかる」と言われながらも、手動ビルドの時代には二度と戻れなくなった歴史の必然と同じ構造です。

---

## まとめ：安全なハーネスの上で自律ループを回す設計
 
ループエンジニアリングは、人間が手作業で行っていた「確認・合否判定・やり直し指示」を、AI と自動検証の仕組みに委ねるアプローチです。

安全なハーネス（隔離と可逆性の保証）を大前提として、**外部記憶・自走権限・客観的判定ロジック** という 3 つの足場を整え、AI 自身に検証と自己進化のサイクルを実行させます。
課題や運用コストも伴うアプローチですが、人間による手動介入のボトルネックを解消し、プロンプトの工夫から「自走する仕組みの設計」へと移行するための実践的なアプローチとして紹介しました。
